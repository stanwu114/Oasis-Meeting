...
/**
 * dsh 历史会话列表:直接读取 DSH_HOME 的会话存储
 * (~/.dsh/sessions/<工作区目录>/session-<uuid>/session.jsonl.zstd),
 * 用 zstd 解压后取最后一条 session/title 事件作为标题。
 */
      ...
      tasks.push(
        (async () => {
          ...
          try {
            ...
            // 标题从压缩日志里取最后一条 session/title;失败则用文件时间兜底
            let title = ''
